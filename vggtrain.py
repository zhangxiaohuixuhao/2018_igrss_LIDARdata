...
import time
from data_input import lidar_train_data2
...
def lidartrain():
    ...
    for i in range(total_number):
        print "i " + str(i)
        lidar, label = lidar_train_data2(batch_size=batch_size_LID, patch_size=patch_size_LID)
        model.fit(lidar, label, epochs=5, batch_size=batch_size_LID)
        if i % save_number == 0:
            print "save model......"
            model.save_weights('model/lidartwo_model.h5')
    K.clear_session()
    t1 = time.time()
